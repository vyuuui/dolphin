...
public:
  Lexer(std::string_view str) : m_lex_string(str), m_match_rule(IdentifierMatchRule::Typical) {}
  ...
  Tagged<CursorPosition, AssemblerToken> const& LookaheadTagRef(size_t num_fwd) const;
  ...
  void LookaheadN(std::array<AssemblerToken, N>& tokens_out) const
  {
    ...
    std::generate_n(tokens_out.begin(), filled_amt, [&_it] { return value_of(*_it++); });

    std::generate_n(tokens_out.begin() + filled_amt, N - filled_amt, [this] {
      CursorPosition p = m_pos;
      return value_of(m_lexed_tokens.emplace_back(p, LexSingle()));
    });
  }
...
private:
  ...
  mutable CursorPosition m_pos, m_scan_pos;
