...
FailureOr<u32>
FillInstruction(MnemonicDesc const& desc, OperandList const& operands, std::string_view inst_line)
{
  ...
  for (u32 i = 0; i < operands.count; i++)
  {
    if (!desc.operand_masks[i].Fits(operands[i]))
    {
      ...
      if (trunc_bits == 0)
      {
        ...
        {
          message = 
            fmt::format("{:#x} not between {:#x} and {:#x}",
        ...
        else
        {
          message =
            fmt::format("{:#x} not between {:#x} and {:#x}",
                        operands[i],
                        desc.operand_masks[i].MinVal(),
                        desc.operand_masks[i].MaxVal());
        }
      }
      else
      {
        ...
        {
          message = 
            fmt::format("{:#x} not between {:#x} and {:#x} or not aligned to {}",
                        static_cast<s32>(operands[i]),
                        static_cast<s32>(desc.operand_masks[i].MinVal()),
                        static_cast<s32>(desc.operand_masks[i].MaxVal()),
                        trunc_bits + 1);
      ...
      return AssemblerError {
        ...
        "",
        0,
        tag_of(operands.list[i]).begin,
        tag_of(operands.list[i]).len
      };
    }
...
void AdjustOperandsForGas(GekkoMnemonic mnemonic, OperandList& ops_list)
{
  switch (mnemonic)
  {
    case GekkoMnemonic::kCmp:
    case GekkoMnemonic::kCmpl:
    case GekkoMnemonic::kCmpi:
    case GekkoMnemonic::kCmpli:
      if (ops_list.count < 4)
      {
        ops_list.Insert(0, 0);
      }
    ...
    case GekkoMnemonic::kAddis:
      // Because GAS wants to allow for addis and lis to work nice with absolute addresses, the immediate operand
      // should also "fit" into the _UIMM field, so just turn a valid UIMM into a SIMM
...
FailureOr<std::vector<CodeBlock>>
Assemble(std::string_view instruction, u32 current_instruction_address)
{
  FailureOr<detail::GekkoIR> parse_result =
    detail::ParseToIR(instruction, current_instruction_address);
  if (is_failure(parse_result))
  {
    return get_failure(parse_result);
  }

  auto const& parsed_blocks = get_t(parse_result).blocks;
  auto const& operands = get_t(parse_result).operand_pool;
  std::vector<CodeBlock> out_blocks;

  for (detail::IRBlock const& parsed_block : parsed_blocks)
  {
    CodeBlock& new_block = out_blocks.emplace_back(parsed_block.block_address);
    for (detail::ChunkVariant const& chunk : parsed_block.chunks)
    {
      if (std::holds_alternative<detail::InstChunk>(chunk))
      {
        for (detail::GekkoInstruction const& parsed_inst : std::get<detail::InstChunk>(chunk))
        {
          OperandList adjusted_ops;
          ASSERT(parsed_inst.op_count <= kMaxOperands);
          adjusted_ops.Copy(operands.begin() + parsed_inst.op_index,
                            operands.begin() + parsed_inst.op_index + parsed_inst.op_count);
          ...
          if (is_failure(inst))
          {
            get_failure(inst).error_line = parsed_inst.raw_text;
            get_failure(inst).line = parsed_inst.line_number;
            return get_failure(inst);
          }

          new_block.PushBigEndian(get_t(inst));
        }
      }
      else if (std::holds_alternative<detail::ByteChunk>(chunk))
      {
        detail::ByteChunk byte_arr = std::get<detail::ByteChunk>(chunk);
        new_block.instructions.insert(new_block.instructions.end(), byte_arr.begin(), byte_arr.end());
      }
