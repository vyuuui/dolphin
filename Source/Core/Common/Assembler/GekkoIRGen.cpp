...
private:
  ...
  std::set<std::string, std::less<>> m_symset;
...
{
  if (m_symset.contains(name))
  {
    m_owner->EmitErrorHere(fmt::format("Label/Constant {} is already defined", name));
    return;
  }

  m_labels[std::string(name)] = m_active_block->BlockEndAddress();
  m_symset.insert(std::string(name));
}
...
void GekkoIRPlugin::OnVarDecl(std::string_view name)
{
  ...
  m_active_var = &m_constants[std::string(name)];
  m_symset.insert(std::string(name));
}
...
void GekkoIRPlugin::RunFixups()
{
  ...
  {
    value_of(m_output_result.operand_pool[i]) = m_operand_fixups[i]();
    if (m_owner->error)
    {
      return;
    }
...
{
  m_build_inst.op_index = m_operand_scan_begin;
  m_build_inst.op_count = m_output_result.operand_pool.size() - m_operand_scan_begin;
