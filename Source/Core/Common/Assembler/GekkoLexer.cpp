...
{
  return m_lexed_tokens.empty() ? m_pos.line : tag_of(m_lexed_tokens.front()).line;
}

size_t Lexer::ColNumber() const
{
  return m_lexed_tokens.empty() ? m_pos.col : tag_of(m_lexed_tokens.front()).col;
}

std::string_view Lexer::CurrentLine() const
{
  const size_t line_index =
      m_lexed_tokens.empty() ? m_pos.index : tag_of(m_lexed_tokens.front()).index;
...
Tagged<CursorPosition, AssemblerToken> const& Lexer::LookaheadTagRef(size_t num_fwd) const
{
...
AssemblerToken Lexer::Lookahead() const
{
  ...
  return value_of(m_lexed_tokens.front());
}
...
void Lexer::FeedbackTokens() const
{
  ...
  m_pos = m_scan_pos = tag_of(m_lexed_tokens.front());
