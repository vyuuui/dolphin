...
CaseInsensitiveDict<u32, '_'> sprg_map = {
...
CaseInsensitiveDict<GekkoDirective> directives_map = {
...
CaseInsensitiveDict<ParseInfo, '.', '_'> mnemonic_tokens = {
...
CaseInsensitiveDict<ParseInfo, '.', '_', '+', '-'> extended_mnemonic_tokens = {
...
#define EMIT_MNEMONIC_ENTRY(opcode_val, extra_bits, ...)                                           \
  MnemonicDesc                                                                                     \
  {                                                                                                \
    insert_opcode(opcode_val) | (extra_bits),                                                      \
...
#define OERC_MNEMONIC(opcode_val, extra_bits, ...)                                                 \
  EMIT_MNEMONIC_ENTRY(opcode_val, extra_bits, __VA_ARGS__),                                        \
      ...
      EMIT_MNEMONIC_ENTRY(                                                                         \
          opcode_val, ((extra_bits) | insert_val(1, 31, 31) | insert_val(1, 21, 21)), __VA_ARGS__)
...
std::array<MnemonicDesc, NUM_MNEMONICS* VARIANT_PERMUTATIONS> mnemonics = {
    // A-2
    ...
    // A-30
    MNEMONIC(4, insert_val(6, 25, 30), _D, _A, _B, _W2, _I2),   // psq_lx
    MNEMONIC(4, insert_val(7, 25, 30), _S, _A, _B, _W2, _I2),   // psq_stx
    MNEMONIC(4, insert_val(38, 25, 30), _D, _A, _B, _W2, _I2),  // psq_lux
    MNEMONIC(4, insert_val(39, 25, 30), _S, _A, _B, _W2, _I2),  // psq_stux
...
template <u32 SPRG>
void fill_mtspr_bat_and_bitswap(OperandList& operands)
{
  operands[0] = spr_bitswap(2 * operands[0] + SPRG);
}

template <u32 SPRG>
void fill_mfspr_bat_and_bitswap(OperandList& operands)
{
  operands[1] = spr_bitswap(2 * operands[1] + SPRG);
}
}  // namespace
...
std::array<ExtendedMnemonicDesc, NUM_EXT_MNEMONICS* VARIANT_PERMUTATIONS> extended_mnemonics = {
    // E.2.1
    PLAIN_PSEUDO(GekkoMnemonic::Addi, negate_SIMM),      // subi
    PLAIN_PSEUDO(GekkoMnemonic::Addis, negate_SIMM),     // subis
    PLAIN_PSEUDO(GekkoMnemonic::Addic, negate_SIMM),     // subic
    PLAIN_PSEUDO(GekkoMnemonic::AddicDot, negate_SIMM),  // subic.

    // E.2.2
    OERC_PSEUDO(GekkoMnemonic::Subf, swap_ops_1_2),   // sub
    OERC_PSEUDO(GekkoMnemonic::Subfc, swap_ops_1_2),  // subc

    // E.3.2
    PLAIN_PSEUDO(GekkoMnemonic::Cmpi, set_compare_word_mode),   // cmpwi
    PLAIN_PSEUDO(GekkoMnemonic::Cmp, set_compare_word_mode),    // cmpw
    PLAIN_PSEUDO(GekkoMnemonic::Cmpli, set_compare_word_mode),  // cmplwi
    PLAIN_PSEUDO(GekkoMnemonic::Cmpl, set_compare_word_mode),   // cmplw
    ...
    // E.5.2
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<12>)),        // bt
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<4>)),         // bf
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bi<16, 0>)),  // bdnz
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<8>)),         // bdnzt
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<0>)),         // bdnzf
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bi<18, 0>)),  // bdz
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<10>)),        // bdzt
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<2>)),         // bdzf
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<13>)),        // bt+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<5>)),         // bf+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bi<17, 0>)),  // bdnz+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<9>)),         // bdnzt+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<1>)),         // bdnzf+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bi<19, 0>)),  // bdz+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<11>)),        // bdzt+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo<3>)),         // bdzf+

    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bi<20, 0>)),  // blr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<12>)),        // btlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<4>)),         // bflr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bi<16, 0>)),  // bdnzlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<8>)),         // bdnztlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<0>)),         // bdnzflr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bi<18, 0>)),  // bdzlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<10>)),        // bdztlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<2>)),         // bdzflr

    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<13>)),        // btlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<5>)),         // bflr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bi<17, 0>)),  // bdnzlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<9>)),         // bdnztlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<1>)),         // bdnzflr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bi<19, 0>)),  // bdzlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<11>)),        // bdztlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo<3>)),         // bdzflr+

    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bi<20, 0>)),  // bctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo<12>)),        // btctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo<4>)),         // bfctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo<13>)),        // btctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo<5>)),         // bfctr+

    // E.5.3
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<12, 0, 2>)),  // blt
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<4, 1, 2>)),   // ble
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<12, 2, 2>)),  // beq
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<4, 0, 2>)),   // bge
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<12, 1, 2>)),  // bgt
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<4, 0, 2>)),   // bnl
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<4, 2, 2>)),   // bne
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<4, 1, 2>)),   // bng
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<12, 3, 2>)),  // bso
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<4, 3, 2>)),   // bns
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<12, 3, 2>)),  // bun
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<4, 3, 2>)),   // bnu

    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<13, 0, 2>)),  // blt+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<5, 1, 2>)),   // ble+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<13, 2, 2>)),  // beq+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<5, 0, 2>)),   // bge+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<13, 1, 2>)),  // bgt+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<5, 0, 2>)),   // bnl+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<5, 2, 2>)),   // bne+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<5, 1, 2>)),   // bng+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<13, 3, 2>)),  // bso+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<5, 3, 2>)),   // bns+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<13, 3, 2>)),  // bun+
    LKAA_PSEUDO(GekkoMnemonic::Bc, (fill_bo_bicond<5, 3, 2>)),   // bnu+

    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<12, 0, 1>)),  // bltlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<4, 1, 1>)),   // blelr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<12, 2, 1>)),  // beqlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<4, 0, 1>)),   // bgelr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<12, 1, 1>)),  // bgtlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<4, 0, 1>)),   // bnllr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<4, 2, 1>)),   // bnelr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<4, 1, 1>)),   // bnglr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<12, 3, 1>)),  // bsolr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<4, 3, 1>)),   // bnslr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<12, 3, 1>)),  // bunlr
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<4, 3, 1>)),   // bnulr

    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<13, 0, 1>)),  // bltlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<5, 1, 1>)),   // blelr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<13, 2, 1>)),  // beqlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<5, 0, 1>)),   // bgelr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<13, 1, 1>)),  // bgtlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<5, 0, 1>)),   // bnllr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<5, 2, 1>)),   // bnelr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<5, 1, 1>)),   // bnglr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<13, 3, 1>)),  // bsolr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<5, 3, 1>)),   // bnslr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<13, 3, 1>)),  // bunlr+
    LK_PSEUDO(GekkoMnemonic::Bclr, (fill_bo_bicond<5, 3, 1>)),   // bnulr+

    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<12, 0, 1>)),  // bltctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<4, 1, 1>)),   // blectr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<12, 2, 1>)),  // beqctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<4, 0, 1>)),   // bgectr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<12, 1, 1>)),  // bgtctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<4, 0, 1>)),   // bnlctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<4, 2, 1>)),   // bnectr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<4, 1, 1>)),   // bngctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<12, 3, 1>)),  // bsoctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<4, 3, 1>)),   // bnsctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<12, 3, 1>)),  // bunctr
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<4, 3, 1>)),   // bnuctr

    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<13, 0, 1>)),  // bltctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<5, 1, 1>)),   // blectr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<13, 2, 1>)),  // beqctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<5, 0, 1>)),   // bgectr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<13, 1, 1>)),  // bgtctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<5, 0, 1>)),   // bnlctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<5, 2, 1>)),   // bnectr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<5, 1, 1>)),   // bngctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<13, 3, 1>)),  // bsoctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<5, 3, 1>)),   // bnsctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<13, 3, 1>)),  // bunctr+
    LK_PSEUDO(GekkoMnemonic::Bcctr, (fill_bo_bicond<5, 3, 1>)),   // bnuctr+
    ...
    // E.7
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<16>),   // twlt
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<16>),  // twlti
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<20>),   // twle
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<20>),  // twlei
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<4>),    // tweq
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<4>),   // tweqi
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<12>),   // twge
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<12>),  // twgei
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<8>),    // twgt
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<8>),   // twgti
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<12>),   // twnl
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<12>),  // twnli
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<24>),   // twne
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<24>),  // twnei
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<20>),   // twng
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<20>),  // twngi
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<2>),    // twllt
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<2>),   // twllti
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<6>),    // twlle
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<6>),   // twllei
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<5>),    // twlge
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<5>),   // twlgei
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<1>),    // twlgt
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<1>),   // twlgti
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<5>),    // twlnl
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<5>),   // twlnli
    PLAIN_PSEUDO(GekkoMnemonic::Tw, trap_set_TO<6>),    // twlng
    PLAIN_PSEUDO(GekkoMnemonic::Twi, trap_set_TO<6>),   // twlngi
    ...
    // E.8
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(1)>),    // mtxer
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(1)>),    // mfxer
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(8)>),    // mtlr
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(8)>),    // mflr
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(9)>),    // mtctr
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(9)>),    // mfctr
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(18)>),   // mtdsisr
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(18)>),   // mfdsisr
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(19)>),   // mtdar
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(19)>),   // mfdar
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(22)>),   // mtdec
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(22)>),   // mfdec
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(25)>),   // mtsdr1
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(25)>),   // mfsdr1
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(26)>),   // mtsrr0
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(26)>),   // mfsrr0
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(27)>),   // mtsrr1
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(27)>),   // mfsrr1
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(280)>),  // mtasr
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(280)>),  // mfasr
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(282)>),  // mtear
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr<spr_bitswap(282)>),  // mfear
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(284)>),  // mttbl
    PLAIN_PSEUDO(GekkoMnemonic::Mftb_nobitswap, fill_mfspr<spr_bitswap(268)>),   // mftbl
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr<spr_bitswap(285)>),  // mttbu
    PLAIN_PSEUDO(GekkoMnemonic::Mftb_nobitswap, fill_mfspr<spr_bitswap(269)>),   // mftbu
    PLAIN_PSEUDO(
        GekkoMnemonic::Mtspr_nobitswap,
        [](OperandList& operands) { operands[0] = spr_bitswap(operands[0] + 272); }),  // mtsprg
    PLAIN_PSEUDO(
        GekkoMnemonic::Mfspr_nobitswap,
        [](OperandList& operands) { operands[1] = spr_bitswap(operands[1] + 272); }),  // mfsprg
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr_bat_and_bitswap<528>),     // mtibatu
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr_bat_and_bitswap<528>),     // mfibatu
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr_bat_and_bitswap<529>),     // mtibatl
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr_bat_and_bitswap<529>),     // mfibatl
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr_bat_and_bitswap<536>),     // mtdbatu
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr_bat_and_bitswap<536>),     // mfdbatu
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, fill_mtspr_bat_and_bitswap<537>),     // mtdbatl
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, fill_mfspr_bat_and_bitswap<537>),     // mfdbatl
    ...
    PLAIN_PSEUDO(GekkoMnemonic::Addi, [](OperandList& operands) { operands.Insert(1, 0); }),  // li
    ...
    PLAIN_PSEUDO(GekkoMnemonic::Mfspr_nobitswap, bitswap_idx<1>),  // mfspr
    PLAIN_PSEUDO(GekkoMnemonic::Mftb_nobitswap, bitswap_idx<1>),   // mfspr
    PLAIN_PSEUDO(GekkoMnemonic::Mtspr_nobitswap, bitswap_idx<0>),  // mtspr
};
...
std::vector<DfaNode> float_dfa = {
    /* 0 */ {{DfaEdge(_plus_or_minus, 1), DfaEdge(_digit, 2), DfaEdge(_dot, 5)},
             "Invalid float: No numeric value"},

    /* 1 */ {{DfaEdge(_digit, 2), DfaEdge(_dot, 5)}, "Invalid float: No numeric value"},

    /* 2 */ {{DfaEdge(_digit, 2), DfaEdge(_dot, 3), DfaEdge(_e, 7)}, std::nullopt},
    ...
    /* 7 */
    {{DfaEdge(_digit, 9), DfaEdge(_plus_or_minus, 8)},
...
std::vector<DfaNode> string_dfa = {
    ...
    /* 0 */ {{DfaEdge(_normal, 0), DfaEdge(_invalid, 1), DfaEdge(_quote, 2), DfaEdge(_escape, 3)},
    ...
    /* 3 */
    {{DfaEdge(_sce, 0), DfaEdge(_invalid, 1), DfaEdge(_octal, 4), DfaEdge(_hexstart, 6)},
     "Invalid string: No terminating \""},

    // Octal characters, at most 3
    /* 4 */
    {{DfaEdge(_normal_minus_octal, 0), DfaEdge(_invalid, 1), DfaEdge(_quote, 2),
      DfaEdge(_escape, 3), DfaEdge(_octal, 5)},
     "Invalid string: No terminating \""},
    /* 5 */
    {{DfaEdge(_normal, 0), DfaEdge(_invalid, 1), DfaEdge(_quote, 2), DfaEdge(_escape, 3)},
     "Invalid string: No terminating \""},

    // Hex characters, 1 or more
    /* 6 */ {{DfaEdge(_hex, 7)}, "Invalid string: bad hex escape"},
    /* 7 */
    {{DfaEdge(_normal_minus_hex, 0), DfaEdge(_invalid, 1), DfaEdge(_quote, 2), DfaEdge(_escape, 3),
      DfaEdge(_hex, 7)},
     "Invalid string: No terminating \""},
};
}  // namespace Common::GekkoAssembler::detail
