...
{
  return
  QStringLiteral("<span style=\"color: red; font-weight: bold\">Error</span> on line %1 col %2")
  .arg(err.line + 1)
  .arg(err.col + 1);
}
...
QString HtmlFormatErrorLine(AssemblerError const& err)
{
  ...
  return
  QStringLiteral("<span style=\"font-family:'monospace';font-size:16px\">"
...
{
  return QStringLiteral("<span>%1</span>")
  .arg(QString::fromStdString(err.message).toHtmlEscaped());
}
  ...
  for (auto const& blk : blocks)
  {
    ...
    for (; i < blk.instructions.size() - 3; i += 4)
    {
      ...
      out_str << fmt::format("{:08x} {:02x}{:02x}{:02x}{:02x}\n", ar_addr,
                             blk.instructions[i], blk.instructions[i + 1],
                             blk.instructions[i + 2], blk.instructions[i + 3]);
    }
...
AssemblerWidget::AssemblerWidget(QWidget* parent)
  : QDockWidget(parent),
  m_reassemble_timer(new QTimer(this)),
  m_unnamed_editor_count(0)
{
  ...
  restoreGeometry(Settings::GetQSettings().value(QStringLiteral("assemblerwidget/geometry")).toByteArray());
  ...
  connect(&Settings::Instance(), &Settings::EmulationStateChanged,
          this, &AssemblerWidget::OnEmulationStateChanged);
...
void AssemblerWidget::CreateWidgets()
{
  ...
  m_assemble = m_toolbar->addAction(tr("Assemble"), this,
                                    [this] { std::vector<CodeBlock> unused; OnAssemble(unused); });
  ...
  m_output_box->setFixedWidth(mono_metrics.horizontalAdvance(QLatin1Char('0')) * kOutputBoxWidth);
  ...
  }
  {
    ...
    output_group->setSizePolicy(QSizePolicy(QSizePolicy::Policy::Fixed, QSizePolicy::Policy::Expanding));
  }
  {
    ...
    error_group->setSizePolicy(QSizePolicy(QSizePolicy::Policy::Expanding, QSizePolicy::Policy::Fixed));
  }
...
void AssemblerWidget::ConnectWidgets()
{
    ...
    [this]
    {
      if (m_output_box->verticalScrollBar()->isVisible())
      {
        m_output_box->setFixedWidth(
            m_output_box->fontMetrics().horizontalAdvance(QLatin1Char('0')) * kOutputBoxWidth +
  ...
  m_copy_output_button->connect(m_copy_output_button, &QPushButton::released,
                                this, &AssemblerWidget::OnCopyOutput);
  m_address_line->connect(m_address_line, &QLineEdit::textChanged,
                          this, &AssemblerWidget::OnBaseAddressChanged);
  m_asm_tabs->connect(m_asm_tabs, &QTabWidget::currentChanged,
                      this, &AssemblerWidget::OnTabChange);
}
...
void AssemblerWidget::OnAssemble(std::vector<CodeBlock>& asm_out)
{
  ...
  switch (m_output_type->currentIndex())
  {
    case 0:
      ...
      break;
    case 1:
      kind = AsmKind::kActionReplay;
      break;
    case 2:
      kind = AsmKind::kGecko;
      break;
    case 3:
      kind = AsmKind::kGeckoExec;
      break;
    case 4:
      kind = AsmKind::kGeckoTrampoline;
      break;
  }
  ...
  if (!good)
  {
    base_address = 0;
    m_error_box->append(
        QStringLiteral("<span style=\"color:#ffcc00\">Warning</span> invalid base address, defaulting to 0"));
  }
...
void AssemblerWidget::OnOpen()
{
  ...
  QStringList paths = DolphinFileDialog::getOpenFileNames(
      ...
      QStringLiteral("%1 (*.s *.S *.asm);;%2 (*)")
        .arg(tr("All Assembly files")).arg(tr("All Files")));
  if (paths.isEmpty())
  {
    return;
  }
...
void AssemblerWidget::OnSave()
{
  ...
  if (save_path.isEmpty())
  {
    ...
    save_path = DolphinFileDialog::getSaveFileName(
        ...
        QStringLiteral("%1;;%2")
          .arg(asm_filter).arg(all_filter), &selected_filter);
    ...
    {
      save_path.append(QLatin1StringView(".S"));
    }
  }
...
void AssemblerWidget::NewEditor(QString const& path)
{
  AsmEditor* new_editor =
    new AsmEditor(path, path.isEmpty() ? AllocateTabNum() : kInvalidEditorNum);
...
void AssemblerWidget::OnTabClose(int index)
{
  ...
  {
    const int result =
      ModalMessageBox::question(this, tr("Unsaved Changes"),
