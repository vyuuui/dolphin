...
AsmEditor::AsmEditor(QString const& path, int editor_num, QWidget* parent)
    : QPlainTextEdit(parent),
      m_path(path),
      m_base_address(QStringLiteral()),
...
int AsmEditor::LineNumberAreaWidth() {
  constexpr int kMinDigitWidth = 2;

  int num_digits = 1;
  for (int max = qMax(1, blockCount()); max >= 10; max /= 10, ++num_digits);
  
  return 3 + CharWidth() * qMax(kMinDigitWidth, num_digits);
}

void AsmEditor::SetBaseAddress(QString const& ba) {
...
bool AsmEditor::PathsMatch(QString const& path) const {
...
bool AsmEditor::SaveFile(QString const& save_path) {
  ...
  if (m_path != save_path) {
    m_path = save_path;
    m_filename = QString::fromStdString(file.filesystemFileName().filename());
    emit PathChanged();
  }
...
void AsmEditor::UpdateLineNumberArea(QRect const& rect, int dy) {
  if (dy != 0) {
...
bool AsmEditor::event(QEvent* e) {
  if (e->type() == QEvent::ToolTip) {
    ...
    QRect check_rect;
    if (info->error_at_eol) {
      hover_cursor.setPosition(hover_block.position() + info->error->col + info->error->len);
      ...
      hover_cursor.setPosition(hover_block.position() + info->error->col);
      const QRect cursor_left = cursorRect(hover_cursor);
      hover_cursor.setPosition(hover_block.position() + info->error->col + info->error->len);
...
void AsmEditor::LineNumberAreaPaintEvent(QPaintEvent* event) {
  ...
  while (block.isValid() && top <= event->rect().bottom()) {
    if (block.isVisible() && bottom >= event->rect().top()) {
      ...
      painter.drawText(0, top, m_line_number_area->width(), fontMetrics().height(),
                       Qt::AlignRight, num);
    }
