...
void CodeViewWidget::DoPatchInstruction(bool assemble)
{
  ...
  {
    run_dialog(
        AssembleInstructionDialog(this, addr, PowerPC::debug_interface.ReadInstruction(addr)),
        addr);
  }
  else
  {
    run_dialog(PatchInstructionDialog(this, addr, PowerPC::debug_interface.ReadInstruction(addr)),
               addr);
  }
}
