...
QString HtmlFormatErrorLine(Common::GekkoAssembler::AssemblerError const& err)
{
  ...
  return
  QStringLiteral("%1<u><span style=\"color:red; font-weight:bold\">%2</span></u>%3")
  .arg(line_pre_error)
  .arg(line_error)
  .arg(line_post_error);
}
...
void AssembleInstructionDialog::OnEditChanged()
{
  ...
  else
  {
    ...
    m_error_loc_label->setText(QStringLiteral("<span style=\"color: green; font-weight: bold\">Ok</span>"));
    m_error_line_label->clear();
